  ...
  try{
    ...
    stage('Clone Project'){
      ...
      git branch: branchName,
          ...
          url: 'http://ec2-18-211-180-13.compute-1.amazonaws.com/sami/mypipeline.git'
    }
    ...
    if (branchName == "dev"){
      ...
      stage('Development Environment'){
        ...
        stage('Development Environment - Find vulnerability'){
          ...
          stage('Development Environment - JMETER Testing'){
            git branch: branchName,
            credentialsId: 'gitlab_connection_sami',
            url: 'http://ec2-18-211-180-13.compute-1.amazonaws.com/sami/docker-jmeter.git'
            sh "mv /tmp/jmeter/jmeter_${branchName}.jmx ${WORKSPACE}"
            sh "./run.sh -n -t jmeter_${branchName}.jmx -l results_${branchName}.jtl"
            sh "cat results_${branchName}.jtl"

            perfReport errorFailedThreshold: 50, errorUnstableThreshold: 10, filterRegex: '', sourceDataFiles: "results_${branchName}.jtl"

            sh "docker rm -vf jmeter"
    ...
    if (branchName == "master"){
      ...
      stage('Production Environment'){
        ...
        stage('Production Environment - JMETER Testing'){
          git branch: branchName,
              credentialsId: 'gitlab_connection_sami',
                url: 'http://ec2-18-211-180-13.compute-1.amazonaws.com/sami/docker-jmeter.git'
          ...
          perfReport errorFailedThreshold: 20, errorUnstableThreshold: 0, filterRegex: '', sourceDataFiles: "results_${branchName}.jtl"
        }
      }
    }
